...
ozpIwc.api.intents.Api = (function (ozpIwc) {
    /**
     * The Intents Api.
     ...
     */
    var Api = ozpIwc.api.createApi("intents.api", function (config) {
        /**
    ...
    Api.prototype.initializeData = function (deathScream) {
        ...
        deathScream.data.forEach(function (packet) {
            if (packet.resource.indexOf("/inFlightIntent") === 0) {
                ...
                this.createNode({
                    ...
                    handlerChoices: [0, 1],
                    state: "deserialize"
                }, ozpIwc.api.intents.InFlightNode).deserializeLive(packet);
            } else {
        ...
        if (this.endpoints) {
            ...
            return Promise.all(this.endpoints.map(function (u) {
                var e = ozpIwc.api.endpoint(u.link);
...
//====================================================================
    ...
    Api.prototype.invokeIntentHandler = function (packet, type, action, handlers, pattern) {
        var self = this;
        var inflightNode = new ozpIwc.api.intents.InFlightNode({
        ...
        this.data[inflightNode.resource] = ozpIwc.api.intents.FSM.transition(inflightNode);
                    ...
                    if (inFlightNode.entity.state === "complete") {
                        ...
                        var onComplete = function (node) {
                            if (node.resource === inFlightNode.resource) {
                                ozpIwc.api.intents.FSM.off("complete", onComplete);
                                res(true);
                            }
                        };
                        var onError = function (node) {
                            if (node.resource === inFlightNode.resource) {
                                ozpIwc.api.intents.FSM.off("error", onError);
                                rej("err");
                            }
                        };
                        ozpIwc.api.intents.FSM.on("complete", onComplete);
                        ozpIwc.api.intents.FSM.on("error", onError);
                        return promise;
                    }
                });
            };
        ...
        var showChooser = function (err) {
            ...
            return useRegisteredChooser(node).catch(function (err) {
                ...
                if (ozpIwc.util.getInternetExplorerVersion() !== 11) {
                    ...
                    node = ozpIwc.api.intents.FSM.transition(node, {state: "error"});
                }
            });
        };
        var self = this;
    ...
    Api.declareRoute({
        ...
        filters: ozpIwc.api.filter.standard.setFilters(ozpIwc.api.intents.InFlightNode)
    }, function (packet, context, pathParams) {
        context.node = ozpIwc.api.intents.FSM.transition(context.node, packet);
...
//====================================================================
    ...
    Api.declareRoute({
        action: "set",
        resource: "/{major}/{minor}/{action}/{handlerId}",
        filters: ozpIwc.api.filter.standard.setFilters(ozpIwc.api.intents.HandlerNode, "application/vnd.ozp-iwc-intent-handler-v1+json")
    }, function (packet, context, pathParams) {
...
//====================================================================
    ...
    Api.registerDefinitionFilter = function (nodeType, contentType) {
        ...
        var filters = ozpIwc.api.filter.standard.setFilters(nodeType, contentType);
        filters.unshift(ozpIwc.api.filter.base.fixPattern());
    ...
    Api.registerHandlerFilter = function (nodeType, contentType) {
        ...
        var handlerFilter = ozpIwc.api.filter.standard.setFilters(nodeType, contentType);
    ...
    Api.declareRoute({
        ...
        var childNode = this.createNode({
            ...
            'src': packet.src
        }, ozpIwc.api.intents.HandlerNode);
    ...
    Api.declareRoute({
        action: "invoke",
        resource: "/{major}/{minor}/{action}",
        filters: ozpIwc.api.filter.standard.getFilters()
    }, function (packet, context, pathParams) {
        return this.invokeIntentHandler(
            packet,
            pathParams.major + "/" + pathParams.minor,
