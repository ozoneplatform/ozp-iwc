...
ozpIwc.api.intents.FSM = (function (ozpIwc) {
    /**
    ...
    FSM.events = new ozpIwc.util.Event();
...
//===============================================
    ...
    FSM.states.delivering = function (entity) {
        if (!entity.handler || !entity.handler.resource || !entity.handler.reason) {
            throw new ozpIwc.api.error.BadStateError("Choosing state requires a resource and reason");
        }
    ...
    FSM.states.running = function (entity) {
        if (!entity.handler || !entity.handler.address) {
            throw new ozpIwc.api.error.BadContentError("Entity lacks a 'handler.address' field");
        }
...
//===============================================
   ...
   FSM.transition = function (node, packet) {
        ...
        if (!packet.entity || !packet.entity.state) {
            throw new ozpIwc.api.error.BadContentError("Entity lacks a 'state' field");
        }
        if (node.deleted) {
            throw new ozpIwc.api.error.BadContentError("Already handled.");
        }
        ...
        if (!transist) {
            throw new ozpIwc.api.error.BadStateError("In-flight intent cannot transition from " +
